...
export const likeService = {
  ...
  toggleVideoLike: async (videoId: string): Promise<ApiResponse<unknown>> => {
    const { data } = await api.post(`/likes/toggle/v/${videoId}`)
    return data
  },

  /** Toggle like on a comment */
  toggleCommentLike: async (commentId: string): Promise<ApiResponse<unknown>> => {
    const { data } = await api.post(`/likes/toggle/c/${commentId}`)
    return data
  },

  /** Toggle like on a tweet */
  toggleTweetLike: async (tweetId: string): Promise<ApiResponse<unknown>> => {
