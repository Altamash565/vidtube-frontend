...
export const TweetItem: React.FC<{ tweet: Tweet; channelAvatar: string }> = ({ tweet, channelAvatar }) => {
  ...
  const [dislikesCount, setDislikesCount] = useState(tweet.dislikes || 0)

  // Sync props change
  useEffect(() => {
    setIsLiked(tweet.isLiked ?? false)
    setLikesCount(tweet.likes || 0)
    setDislikesCount(tweet.dislikes || 0)
  }, [tweet.id, tweet.isLiked, tweet.likes, tweet.dislikes])
      ...
      <div className="w-full text-left">
        ...
        <div className="flex gap-4">
          ...
          >
            ...
            <span>{Math.max(0, isDisliked ? dislikesCount + 1 : dislikesCount)}</span>
          </button>
        </div>
      </div>
    </div>
