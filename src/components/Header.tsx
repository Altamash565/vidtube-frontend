...
export interface HeaderProps {
  ...
  userChannel?: { name: string; handle: string; avatar: string } | null
  onLoginClick?: () => void
  onSignupClick?: () => void
  onLogout?: () => void
  onMyChannelClick?: () => void
    ...
    <header className="sticky inset-x-0 top-0 z-50 w-full border-b border-white bg-[#121212] px-4">
      <nav className="mx-auto flex max-w-7xl items-center py-2">
        ...
        <button 
          ...
          className="ml-4 flex w-6 shrink-0 flex-wrap gap-y-1.5 sm:hidden"
        >
          <span className="block h-[2px] w-full bg-white"></span>
          <span className="block h-[2px] w-2/3 bg-white"></span>
          <span className="block h-[2px] w-full bg-white"></span>
        </button>

        {/* Navigation Sidebar/Dropdown Container */}
        <div 
          className={`fixed inset-y-0 right-0 flex w-full max-w-xs shrink-0 flex-col border-l border-white bg-[#121212] transition-transform duration-200 z-50 sm:static sm:ml-4 sm:w-auto sm:translate-x-0 sm:border-none ${
        ...
        >
          ...
          {/* Extra Links for Mobile Dropdown */}
          <ul className="my-4 flex w-full flex-wrap gap-2 px-4 sm:hidden">
            ...
            <li className="w-full">
              <button 
                ...
                className="flex w-full items-center justify-start gap-x-4 border border-white px-4 py-1.5 text-left hover:bg-[#ae7aff] hover:text-black focus:border-[#ae7aff] focus:bg-[#ae7aff] focus:text-black w-full"
              >
                <span className="inline-block w-full max-w-[20px]">
                  <svg style={{ width: '100%' }} viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          {isLoggedIn ? (
            <div className="mb-8 mt-auto flex w-full flex-col sm:flex-row gap-4 px-4 sm:mb-0 sm:mt-0 sm:items-center sm:px-0 sm:w-auto relative">
              {/* Channel User Avatar / Mobile info */}
              <button 
                onClick={() => {
                  if (window.innerWidth < 640) {
                    setIsMobileMenuOpen(false)
                    if (onMyChannelClick) onMyChannelClick()
                  } else {
                    setIsDropdownOpen(!isDropdownOpen)
                  }
                }}
                className="flex w-full gap-4 text-left sm:items-center outline-none"
              >
                <img 
                  src={userChannel?.avatar || 'https://images.pexels.com/photos/1115816/pexels-photo-1115816.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1'} 
                  alt={userChannel?.name || 'React Patterns'} 
                  className="h-16 w-16 shrink-0 rounded-full sm:h-12 sm:w-12 object-cover border border-neutral-850"
                />
                <div className="w-full pt-2 sm:hidden">
                  <h6 className="font-semibold text-white">{userChannel?.name || 'React Patterns'}</h6>
                  <p className="text-sm text-gray-350">{userChannel?.handle || '@reactpatterns'}</p>
                </div>
              </button>

              {/* Mobile Logout Button (Visible only on mobile inside the drawer) */}
              <div className="sm:hidden w-full border-t border-neutral-850 pt-3 mt-1">
                <button 
                  onClick={() => {
                    setIsMobileMenuOpen(false)
                    if (onLogout) onLogout()
                  }}
                  className="w-full bg-neutral-900 border border-neutral-850 text-red-400 hover:text-red-300 font-bold px-3 py-2 text-sm rounded cursor-pointer transition-colors"
                >
                  Log out
                </button>
              </div>
              ...
              {isDropdownOpen && (
                <div className="hidden sm:block absolute right-0 top-full mt-2.5 w-56 rounded-xl border border-neutral-800 bg-[#161616] p-3.5 shadow-2xl z-[60] text-left">
                  <p className="text-xs text-neutral-400 font-medium mb-0.5">Logged in as</p>
                  <p className="text-sm text-white font-semibold truncate mb-0.5" title={userChannel?.name || ''}>
                    {userChannel?.name || 'React Patterns'}
                  </p>
                  <p className="text-xs text-neutral-400 truncate mb-3 border-b border-neutral-800 pb-2.5" title={userChannel?.handle || ''}>
                    {userChannel?.handle || '@reactpatterns'}
                  </p>
                  <button 
                    onClick={() => {
                    ...
                    className="w-full text-left px-3 py-2 text-sm text-neutral-250 hover:text-white hover:bg-neutral-900 rounded-lg transition-colors font-semibold cursor-pointer mb-1"
                  >
                    View Channel
                  </button>
                  <button 
