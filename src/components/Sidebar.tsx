...
export const Sidebar: React.FC<SidebarProps> = ({ activeId = 'home', onSelect }) => {
  const items: SidebarItem[] = [
    ...
    },
    {
      ...
      icon: (
        <svg style={{ width: '100%' }} viewBox="0 0 22 20" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M20.7 11.5L18.7005 9.5L16.7 11.5M19 10C19 14.9706 14.9706 19 10 19C5.02944 19 1 14.9706 1 10C1 5.02944 5.02944 1 10 1C13.3019 1 16.1885 2.77814 17.7545 5.42909M10 5V10L13 12" stroke="currentColor" strokeWidth="2" stroke-linecap="round" stroke-linejoin="round"></path>
        </svg>
      )
    },
    {
    ...
    },
    {
      ...
      icon: (
        <svg style={{ width: '100%' }} viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M11 14C12.6569 14 14 12.6569 14 11C14 9.34315 12.6569 8 11 8C9.34315 8 8 9.34315 8 11C8 12.6569 9.34315 14 11 14Z" stroke="currentColor" strokeWidth="2" stroke-linecap="round" stroke-linejoin="round"></path>
  ...
  return (
    <aside className="group fixed inset-x-0 bottom-0 z-40 w-full shrink-0 border-t border-white bg-[#121212] px-2 py-2 sm:absolute sm:inset-y-0 sm:max-w-[70px] sm:border-r sm:border-t-0 sm:py-6 sm:hover:max-w-[250px] lg:sticky lg:max-w-[250px] transition-all duration-300">
      <ul className="flex justify-around gap-y-2 sm:sticky sm:top-[106px] sm:min-h-[calc(100vh-130px)] sm:flex-col">
        {items.map((item) => {
          const isActive = item.id === activeId
          const activeClasses = isActive
            ? 'text-[#ae7aff] border-[#ae7aff] bg-[#ae7aff]/10'
            : 'border-white text-white hover:bg-[#ae7aff] hover:text-black focus:border-[#ae7aff] focus:bg-[#ae7aff] focus:text-black'

          return (
            <li 
              key={item.id} 
              className={`${item.desktopOnly ? 'hidden sm:block' : ''} ${item.marginTopAuto ? 'sm:mt-auto' : ''}`}
            >
              <button
                onClick={() => onSelect && onSelect(item.id)}
                className={`flex flex-col items-center justify-center py-1 sm:w-full sm:flex-row sm:border sm:p-1.5 sm:group-hover:justify-start sm:group-hover:px-4 lg:justify-start lg:px-4 transition-all duration-150 ${activeClasses}`}
              >
                <span className="inline-block w-5 shrink-0 sm:group-hover:mr-4 lg:mr-4">
                  {item.icon}
                </span>
                <span className="block text-[10px] sm:text-sm sm:hidden sm:group-hover:inline lg:inline font-medium">
                  {item.label}
                </span>
              </button>
            </li>
